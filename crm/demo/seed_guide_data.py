...
def _save_quote(deal, facilities, addons, payment_terms="Annual Upfront", contract_term_yrs=3, notes=None):
	"""
		...
		lines.append(
			{
				...
				"facility_name": fname,
				"package_tier": f.get("package_tier", ""),
			}
		)
		lines.append(
			{
...
def run():
	...
	deal_knh = deals_by_org.get("Kenyatta National Hospital") or org_to_deal.get("Kenyatta National Hospital")
	if deal_knh:
	...
	deal_nwh = deals_by_org.get("Nairobi Women's Hospital") or org_to_deal.get("Nairobi Women's Hospital")
	if deal_nwh:
