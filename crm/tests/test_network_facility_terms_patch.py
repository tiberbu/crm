...
class TestNetworkFacilityTermsPatch(UnitTestCase):
	def test_creates_reviewable_template_without_changing_settings(self):
		...
		self.assertIn("{{ contract_totals.monthly_exclusive_vat_display }}", payload["terms"])
		self.assertIn("{{ contract_totals.monthly_vat_display }}", payload["terms"])
		self.assertIn("{{ contract_totals.selected_term_exclusive_vat_display }}", payload["terms"])
		self.assertIn("{{ contract_totals.selected_term_vat_display }}", payload["terms"])
