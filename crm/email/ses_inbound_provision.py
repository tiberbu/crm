...
def _ses_ensure_rule_set(ses_client) -> None:
	existing = [rs["Name"] for rs in ses_client.list_receipt_rule_sets().get("RuleSets", [])]
...
def _ses_upsert_rule(
	...
	existing_rules = ses_client.describe_receipt_rule_set(RuleSetName=_RULE_SET).get("Rules", [])
