...
def create_sales_invoice_from_quotation(quotation_name):
	...
	if frappe.db.get_value("Quotation", quotation_name, "docstatus") != 1:
		frappe.throw(
			"Quotation %s must be submitted (docstatus=1) before creating a Sales Invoice" % quotation_name
		)
...
def create_sales_invoice(quote):
	...
	frappe.log_error(
		"create_sales_invoice called on legacy CRM Quote %s — should use create_sales_invoice_from_quotation"
		% quote.name,
