<template>
  <div class="fc-scoped-view space-y-4">
    <!-- Read-only notice -->
    <div
      class="flex items-center gap-2 px-4 py-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800"
    >
      <LucideInfo class="w-4 h-4 text-blue-500 shrink-0" />
      <span class="text-sm text-blue-700 dark:text-blue-300"
        >You have read-only access to your team's commission records.</span
      >
    </div>
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex flex-col gap-1"
      >
        <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
          <LucideDollarSign class="w-4 h-4" />
          <span class="text-xs uppercase tracking-wide"
            >Unpaid Commissions</span
          >
        </div>
        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">
          {{
            formatCurrency(
              kpiData.unpaid_commissions?.value,
              kpiData.unpaid_commissions?.currency,
            )
          }}
        </div>
      </div>
...
const commissionsResource = createResource({
  ...
  makeParams() {
    return { company: company.value, page_size: 50 }
  },
  auto: true,
})
