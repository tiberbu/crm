<template>
  <div class="space-y-3">
    ...
    >
      ...
      <p class="text-sm">
        Expense Claims and Employee Advances require the HRMS app.
      </p>
    </div>
    ...
    <template v-else>
      ...
      >
        <template #actions="{ row }">
          ...
          >
            {{ markingPaid === row.name ? 'Saving…' : 'Mark Paid' }}
          </button>
          <span
            v-else
...
const markPaidResource = createResource({
  url: 'crm.finance.api.mark_expense_claim_paid',
})
