  ...
  <div class="crm-dashboard space-y-5">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      >
        ...
        <div
          v-if="loading"
          class="h-8 w-24 rounded-md bg-surface-gray-2 animate-pulse"
        />
        <div v-else class="text-2xl font-bold text-ink-gray-9 tabular-nums">
          {{ tile.formatted }}
        </div>
          ...
          <template v-if="!loading && tile.delta !== 0">
            <span
              :class="[
                'size-3',
                tile.delta > 0
                  ? 'lucide-trending-up text-ink-green-6'
                  : 'lucide-trending-down text-ink-red-6',
              ]"
            />
            <span
              :class="tile.delta > 0 ? 'text-ink-green-6' : 'text-ink-red-6'"
              >{{ Math.abs(tile.delta) }}%</span
            >
    ...
    <div class="fc-glass-card !p-0 overflow-hidden">
      ...
      <div
        v-if="loading"
        class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-outline-gray-1/40"
      >
      ...
      >
        ...
        >
          <div class="flex items-center gap-1.5 mb-1.5">
            ...
            <span
              class="text-xs font-medium text-ink-gray-5 group-hover:text-ink-gray-7 transition-colors"
              >{{ stage.label }}</span
            >
          </div>
          ...
          <p class="text-xs text-ink-gray-5 tabular-nums mt-0.5">
            {{ stage.amount }}
          </p>
        </div>
      </div>
    ...
    <div class="fc-glass-card !p-0 overflow-hidden">
      ...
      <div v-else class="divide-y divide-outline-gray-1/40">
        ...
        >
          <span
            class="w-32 font-mono text-xs font-semibold text-ink-gray-7 truncate flex-shrink-0"
            >{{ inv.name }}</span
          >
          ...
          <span
            :class="[
              ...
              'inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 w-24 justify-center',
            ]"
          >
            {{ inv.status }}
          </span>
...
const kpiTiles = computed(() => [
  {
    ...
    tone: TONES.neutral,
    formatted: fmtKpi('ar_outstanding'),
    delta: delta('ar_outstanding'),
  },
  {
    key: 'ar_overdue',
    label: 'AR Overdue',
    ...
    formatted: fmtKpi('ar_overdue'),
    delta: delta('ar_overdue'),
  },
  {
    key: 'invoiced_mtd',
    label: 'Invoiced MTD',
    ...
    tone: TONES.pending,
    formatted: fmtKpi('invoiced_mtd'),
    delta: delta('invoiced_mtd'),
  },
  {
    key: 'collected_mtd',
    label: 'Collected MTD',
    ...
    tone: TONES.positive,
    formatted: fmtKpi('collected_mtd'),
    delta: delta('collected_mtd'),
  },
])
...
const pipelineStages = computed(() => [
  {
    ...
    label: 'Open Quotes',
    section: 'quotes',
    dot: 'bg-surface-gray-6',
    count: pd.value.open_quotes ?? '—',
    amount: fmtCurrency(pd.value.quotes_value ?? 0, pd.value.currency),
  },
  {
    key: 'orders',
    label: 'Active Orders',
    ...
    count: pd.value.active_orders ?? '—',
    amount: fmtCurrency(pd.value.orders_value ?? 0, pd.value.currency),
  },
  {
    key: 'invoices',
    label: 'Unpaid Invoices',
    section: 'invoices',
    dot: 'bg-surface-red-6',
    count: pd.value.unpaid_invoices ?? '—',
    amount: fmtCurrency(pd.value.invoices_value ?? 0, pd.value.currency),
  },
  {
    key: 'payments',
    label: 'Received MTD',
    ...
    count: pd.value.payments_mtd ?? '—',
    amount: fmtCurrency(pd.value.payments_value ?? 0, pd.value.currency),
  },
])
