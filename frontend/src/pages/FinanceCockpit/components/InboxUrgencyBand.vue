<template>
  <div v-if="items.length" class="mb-4">
    ...
    >
      ...
      <span>{{ label }}</span>
      <span class="ml-auto bg-white/20 rounded-full px-1.5 py-0.5 text-xs">{{
        items.length
      }}</span>
    </div>
    <div
      class="border border-t-0 rounded-b border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700"
    >
      <slot />
    </div>
  </div>
...
const BAND_CONFIG = {
  ...
  warning: {
    ...
    bandIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>',
  },
  normal: {
    label: 'Normal',
