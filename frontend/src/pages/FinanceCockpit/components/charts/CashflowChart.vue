<template>
  <div class="fc-cashflow-chart">
    ...
    <div
      ...
      class="h-44 flex items-center justify-center text-sm text-gray-400"
    >
      No data
    </div>
...
const gridYs = computed(() => {
  ...
  return Array.from(
    { length: steps + 1 },
    (_, i) => PAD_T + ((svgH - PAD_T - PAD_B) * i) / steps,
  )
})
...
const linePathInflow = computed(() => {
  ...
  return props.data
    .map((d, i) => (i === 0 ? 'M' : 'L') + xAt(i) + ' ' + yAt(d.inflow))
    .join(' ')
})
const linePathOutflow = computed(() => {
  if (!props.data.length) return ''
  return props.data
    .map((d, i) => (i === 0 ? 'M' : 'L') + xAt(i) + ' ' + yAt(d.outflow))
    .join(' ')
})
