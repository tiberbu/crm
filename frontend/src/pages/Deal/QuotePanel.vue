  ...
  <div class="mt-4">
    ...
    <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
      <div class="flex items-center gap-2">
        ...
        <span v-if="data?.name" class="font-mono text-xs text-ink-gray-4">{{
          data.name
        }}</span>
      </div>
      <div class="flex items-center gap-2">
        ...
        <span v-else class="text-xs text-ink-gray-5">
          {{ __('Price List') }}: {{ data?.price_list || '—' }}
        </span>
        <span v-if="data?.status" :class="pillClass(data.status)">{{
          __(data.status)
        }}</span>
      </div>
    </div>
      ...
      <template v-else>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            >
              <tr>
                <th class="px-3 py-2.5 text-left font-medium w-8">
                  {{ __('#') }}
                </th>
                <th class="px-3 py-2.5 text-left font-medium">
                  {{ __('Item') }}
                </th>
                <th class="px-3 py-2.5 text-right font-medium w-20">
                  {{ __('Qty') }}
                </th>
            ...
            <tbody class="divide-y divide-outline-elevation-2">
              <tr v-for="(line, i) in lines" :key="i" class="align-top">
                ...
                <td class="px-3 py-3">
                  ...
                  <div class="mt-0.5 font-mono text-xs text-ink-gray-4">
                    {{ line.item_code }}
                  </div>
                  <div
        ...
        >
          <div class="w-72 space-y-1.5 text-sm">
            ...
            <div
              class="flex justify-between rounded-lg bg-red-600 px-3 py-2 font-bold text-white"
            >
        ...
        >
          <p v-if="!data?.editable" class="text-xs text-ink-gray-5">
            {{
              __('This quote has been {0} and can no longer be edited.', [
                data?.status,
              ])
            }}
          </p>
          ...
          <span
            ...
            class="text-xs"
            :class="
              dirty ? 'text-amber-700 dark:text-amber-400' : 'text-ink-gray-4'
            "
          >
...
const switchListResource = createResource({
  url: 'crm.api.quotes.set_quote_price_list',
})
...
async function onChangePriceList(priceList) {
  ...
  try {
    ...
    toast.error(
      err?.messages?.[0] ?? err?.message ?? __('Failed to switch price list'),
    )
  } finally {
...
const catalogueOptions = computed(() =>
  (catalogueResource.data ?? []).map((c) => ({
    label: `${c.label} — ${c.item_code}`,
    value: c.item_code,
  })),
)
...
watch(
  ...
  ([editable, switchable]) => {
    ...
    if (!catalogueResource.data && !catalogueResource.loading) {
      catalogueResource.fetch()
    }
  },
  { immediate: true },
)
...
async function save() {
  ...
  try {
    ...
    toast.error(
      err?.messages?.[0] ?? err?.message ?? __('Failed to save quote'),
    )
  } finally {
    saving.value = false
  }
