  ...
  <div class="flex h-full flex-col overflow-hidden">
    ...
    <div class="border-b border-outline-gray-2 px-5 pt-3 pb-3">
      <h1 class="text-xl font-semibold text-ink-gray-9">
        {{ __('All Prequalified Contacts') }}
      </h1>
    </div>

    <!-- Filter bar -->
    <div
      class="flex flex-wrap items-center gap-3 border-b border-outline-gray-2 px-5 py-2.5"
    >
      <select
      ...
      >
        ...
        <option v-for="n in networkRows" :key="n.name" :value="n.slug">
          {{ n.display_name }}
        </option>
      </select>
    ...
    <div class="flex-1 overflow-auto">
      <div
        v-if="facilityListResource.loading"
      ...
      >
        ...
        <p class="mt-1 text-xs text-ink-gray-4">
          {{ __('Adjust filters or add contacts via a network detail page.') }}
        </p>
      </div>
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody class="divide-y divide-outline-elevation-2">
          ...
          >
            <td class="px-5 py-3 font-mono text-xs font-medium text-ink-gray-9">
              {{ row.mfl_code }}
            </td>
            ...
            <td class="px-4 py-3 text-xs text-ink-gray-6">
              {{ (row.memberships ?? [])[0]?.contact_email || '—' }}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <!-- Pagination -->
      <div
        v-if="facilityTotal > facilityPageSize"
        class="flex items-center justify-between border-t border-outline-gray-2 px-5 py-3"
      >
        <span class="text-xs text-ink-gray-5">
          {{
            __('Showing {0}–{1} of {2}', [
              page * facilityPageSize + 1,
              Math.min((page + 1) * facilityPageSize, facilityTotal),
              facilityTotal,
            ])
          }}
        </span>
        <div class="flex gap-2">
          ...
          <Button
            size="sm"
            variant="subtle"
            :disabled="(page + 1) * facilityPageSize >= facilityTotal"
            @click="nextPage"
            >{{ __('Next') }}</Button
          >
        </div>
...
function membershipPill(status) {
  const base =
    'rounded-full px-2 py-0.5 text-xs font-medium transition-colors hover:opacity-80 cursor-pointer'
  const map = {
