<template>
  <div class="flex h-full overflow-hidden bg-surface-gray-1">
    <aside
      class="flex w-80 shrink-0 flex-col border-r border-outline-gray-2 bg-surface-white"
    >
      <div class="border-b border-outline-gray-2 px-5 py-4">
        ...
        <p class="mt-1 text-sm text-ink-gray-5">
          {{
            __('Control the agreement accepted with every Opt-In submission.')
          }}
        </p>
      </div>
      <div class="flex-1 overflow-y-auto p-3">
        ...
        >
          <div class="flex items-start justify-between gap-2">
            <span class="line-clamp-2 font-medium text-ink-gray-9">{{
              document.title
            }}</span>
            <Badge
    ...
    <main class="flex min-w-0 flex-1 flex-col">
        ...
        <div class="min-w-0">
          <div class="flex items-center gap-2">
            <h2 class="truncate text-base font-semibold text-ink-gray-9">
              {{ editor.title || __('Untitled agreement') }}
            </h2>
            <Badge
              v-if="isActive"
              :label="__('Default for Opt-In')"
              theme="green"
              variant="subtle"
            />
          </div>
          <p class="mt-0.5 text-sm text-ink-gray-5">
            {{
              __(
                'The active agreement is rendered when a facility reviews its commitment.',
              )
            }}
          </p>
        </div>
      ...
      >
        <FormControl
          ...
          :required="true"
          :placeholder="__('CareverseHIMS Opt-In Terms and Conditions')"
        />
        <div>
          <label class="mb-1.5 block text-sm font-medium text-ink-gray-7"
            >{{ __('Agreement content') }}
            <span class="text-ink-red-6">*</span></label
          >
          <TextEditor
            ...
            :placeholder="
              __(
                'Write the terms that facilities must accept before submitting an Opt-In request.',
              )
            "
      ...
      <div
        v-else
        class="grid flex-1 place-items-center text-sm text-ink-gray-5"
      >
        {{ __('Create or select a Terms and Conditions document.') }}
      </div>
    </main>
  </div>
</template>
...
const getResource = createResource({
  url: 'crm.api.optin_admin.get_optin_terms',
})
const saveResource = createResource({
  url: 'crm.api.optin_admin.save_optin_terms',
})
const defaultResource = createResource({
  url: 'crm.api.optin_admin.set_default_optin_terms',
})
...
async function loadDocuments(selectName = selectedName.value) {
  ...
  try {
    ...
    toast.error(
      error?.messages?.[0] ??
        error?.message ??
        __('Could not load Terms and Conditions'),
    )
  } finally {
    loading.value = false
  }
...
async function save() {
  ...
  try {
    ...
    toast.success(
      result.normalized
        ? __(
            'Terms and Conditions saved. Unsupported dynamic text was preserved as agreement text.',
          )
    ...
    toast.error(
      error?.messages?.[0] ??
        error?.message ??
        __('Could not save Terms and Conditions'),
    )
  } finally {
    saving.value = false
  }
...
async function setDefault() {
  ...
  try {
    ...
    documents.value = documents.value.map((document) => ({
      ...document,
      active: document.name === activeName.value,
    }))
    ...
    toast.error(
      error?.messages?.[0] ??
        error?.message ??
        __('Could not update the default agreement'),
    )
  } finally {
