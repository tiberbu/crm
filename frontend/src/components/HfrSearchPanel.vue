  ...
  <div v-if="hfrEnabled" class="hfr-panel flex flex-col gap-3">
    ...
    <div class="relative flex items-center">
      ...
      <input
        ...
        v-model="localQuery"
        type="text"
        class="w-full rounded-lg border border-outline-gray-2 bg-surface-white dark:bg-surface-gray-2 pl-9 pr-9 py-2 text-p-sm text-ink-gray-8 placeholder:text-ink-gray-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow"
      ...
      />
      <span
        v-if="searching || fetchingDetail"
        class="absolute right-3 text-ink-gray-4 pointer-events-none"
      >
        <svg
    ...
    <div v-if="results.length" class="flex flex-col gap-1">
      <button
        v-for="r in results"
        :key="r.fid"
        class="group flex items-start gap-3 rounded-lg border border-outline-gray-2 bg-surface-white dark:bg-surface-gray-2 px-3 py-2.5 text-left hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
        :disabled="fetchingDetail"
        @click="select(r)"
      >
        <span
          class="mt-0.5 shrink-0 size-8 rounded-md bg-surface-gray-2 dark:bg-surface-gray-3 flex items-center justify-center text-ink-gray-5 group-hover:bg-blue-100 dark:group-hover:bg-blue-900 transition-colors"
        >
          <svg
            class="size-4"
            viewBox="0 0 16 16"
            ...
            stroke-width="1.25"
          >
            <path d="M2 6.5V13h12V6.5M1 6h14M6 13V9h4v4" />
            <path d="M8 3l5 3H3l5-3z" />
          </svg>
        </span>
        <div class="flex flex-col gap-0.5 min-w-0 flex-1">
          ...
          <div class="flex items-center gap-1.5 flex-wrap">
            <span
              v-if="r.mfl_code"
              class="inline-flex items-center rounded px-1.5 py-0.5 text-p-xs bg-surface-gray-2 dark:bg-surface-gray-3 text-ink-gray-6"
            >
              MFL {{ r.mfl_code }}
            </span>
            <span
            ...
            <span v-if="r.county" class="text-p-xs text-ink-gray-5">{{
              r.county
            }}</span>
            <span v-if="r.owner_type" class="text-p-xs text-ink-gray-4"
              >· {{ r.owner_type }}</span
            >
          </div>
        </div>
        <span
          class="shrink-0 mt-0.5 text-p-xs text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
        >
          {{ __('Add →') }}
        </span>
      </button>
    ...
    <!-- No results -->
    <div
      v-else-if="searched && !searching && localQuery.length >= 3"
      class="flex flex-col items-center gap-1 py-3 text-center"
    >
      <svg
        class="size-7 text-ink-gray-3"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        stroke-width="1.25"
      ...
      <p class="text-p-xs text-ink-gray-4">
        {{ __('Check the code or fill manually') }}
      </p>
    </div>
    ...
    <div v-if="pendingRows.length" class="flex flex-col gap-1">
      ...
      <div
        ...
        class="flex items-center gap-2 rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950 px-3 py-2"
      >
        <div class="flex flex-col flex-1 min-w-0 gap-0.5">
          ...
          <span class="text-p-xs text-ink-gray-5">
            ...
            {{ row.hfr_county }}
            <span v-if="row.facility_owner_type">
              · {{ row.facility_owner_type }}</span
            >
          </span>
        </div>
        <span
          class="shrink-0 text-p-xs bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded px-1.5 py-0.5"
        >
          {{ __('Verified') }}
        </span>
        <button
...
const emit = defineEmits(['row-added', 'row-removed', 'update:facilities'])

const hfrEnabled = inject('hfrEnabled', ref(false))
const facilities = computed(() => props.doc.facilities || [])
...
const detailResource = createResource({
  ...
  onSuccess(data) {
    ...
    const existing = facilities.value.find((r) => r.hfr_facility_id === fid)
    if (existing) {
...
const pendingRows = computed(() => facilities.value.filter((r) => !r.name))
...
function select(r) {
  const already = facilities.value.find((row) => row.hfr_facility_id === r.fid)
  if (already) {
    toast.warning(__('This facility is already in the list.'))
    return
  }
...
function removeRow(fid) {
  emit(
    'update:facilities',
    facilities.value.filter((r) => r.hfr_facility_id !== fid),
  )
