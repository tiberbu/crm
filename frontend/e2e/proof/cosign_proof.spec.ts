...
test('Surface 2: Contracting tab co-signatories (light + dark)', async ({
  browser,
}) => {
  for (const scheme of ['light', 'dark'] as const) {
    ...
    const anchor = page.getByText('Network & Tiberbu Co-Signatories', {
      exact: false,
    })
    ...
    await page.screenshot({
      path: `${SHOTS}/surface2-contracting-${scheme}.png`,
      fullPage: true,
    })
    await ctx.close()
  }
...
test('Surface 3: inline-edit pending signatory (nc-s3-3)', async ({
  browser,
}) => {
  ...
  await page.screenshot({
    path: `${SHOTS}/surface3-inline-edit.png`,
    fullPage: true,
  })
  await ctx.close()
})
...
test('Surface 4: guest signing portal end-to-end (nc-qa)', async ({
  browser,
}) => {
  ...
  try {
    ...
    await page.goto(
      `/sign-contract?contract=${contract}&role=${role.replace(/ /g, '+')}&token=${token}`,
    )
    ...
    await expect(
      page.getByRole('heading', { name: 'Review & Sign' }),
    ...
    await expect(
      page.getByText('Your signature has been recorded.'),
    ).toBeVisible({ timeout: 30000 })
    await page.screenshot({
      path: `${SHOTS}/surface4-portal-signed.png`,
      fullPage: true,
    })
...
test('Surface 1: opt-in Review witness capture (light + dark)', async ({
  browser,
}) => {
  for (const scheme of ['light', 'dark'] as const) {
    const ctx = await browser.newContext({ colorScheme: scheme })
    ...
    await page.screenshot({
      path: `${SHOTS}/debug-after-continue-${scheme}.png`,
      fullPage: true,
    })
    ...
    const loadBtn = page.getByRole('button', {
      name: /Calculate|Load|Get Pricing|Show/i,
    })
    ...
    await page.screenshot({
      path: `${SHOTS}/surface1-review-witness-${scheme}.png`,
      fullPage: true,
    })
    ...
    await page
      .getByPlaceholder('witness@hospital.or.ke')
      .fill('grace.wanjiru@apex.co.ke')
    await page.waitForTimeout(400)
    await page.screenshot({
      path: `${SHOTS}/surface1-review-filled-${scheme}.png`,
      fullPage: true,
    })
    await ctx.close()
  }
